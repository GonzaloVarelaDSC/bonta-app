...
import { PriorityBadge, Avatar, CountdownBadge } from '../Common/Badges';
...
function KanbanColumnView({ colKey, label, jobs }: { colKey: string; label: string; jobs: Job[] }) {
  ...
  function handleDragEnd(e: DragEndEvent) {
    ...
    if (targetStatus === 'LISTO_PARA_ENTREGA' || targetStatus === 'LISTO_PARA_INSTALACION') {
      ...
      if (requiredPending.length > 0) {
        alert(`No se puede pasar a "${targetCol.label}": faltan ${requiredPending.length} ítems obligatorios del control de calidad. Completalos desde la ficha del trabajo.`);
        return;
      }
    }
    const finalStatus = job.requiresInstallation && targetCol.key === 'listo' ? 'LISTO_PARA_INSTALACION' : targetStatus;
    setStatus(job.id, finalStatus, user.id);
  }
        ...
        <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
          <div className="flex gap-4 h-full pb-4" style={{ minWidth: KANBAN_COLUMNS.length * 300 }}>
            {KANBAN_COLUMNS.map((col) => (
              <KanbanColumnView key={col.key} colKey={col.key} label={col.label} jobs={jobs.filter((j) => columnOf(j) === col.key)} />
            ))}
          </div>
        </DndContext>
      </div>
