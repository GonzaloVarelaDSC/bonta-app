...
export function BlockModal({ onClose, onConfirm }: { onClose: () => void; onConfirm: (reason: BlockReason, description: string) => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-ink-950/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-pop w-full max-w-md p-5" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-display font-bold text-ink-900">Bloquear trabajo</h3>
          <button onClick={onClose} className="text-ink-400 hover:text-ink-700"><X size={18} /></button>
        </div>
        <label className="block text-xs font-medium text-ink-600 mb-1.5">Motivo</label>
        <select value={reason} onChange={(e) => setReason(e.target.value as BlockReason)} className="w-full border border-ink-200 rounded-lg px-3 py-2 text-sm mb-4 focus:outline-none focus:ring-2 focus:ring-crit/30">
          {Object.entries(BLOCK_REASON_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
        </select>
        <label className="block text-xs font-medium text-ink-600 mb-1.5">Descripción (obligatoria)</label>
        ...
        <div className="flex gap-2 justify-end">
          <button onClick={onClose} className="px-3.5 py-2 rounded-lg text-sm text-ink-600 hover:bg-ink-50">Cancelar</button>
          <button
