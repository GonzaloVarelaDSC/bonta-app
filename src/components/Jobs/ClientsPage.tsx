          ...
          {clients.map((c) => {
            ...
            return (
              <button key={c.id} onClick={() => setOpenId(c.id)}
                ...
                <div className="text-xs text-ink-400 mt-0.5">{active} activos · {total} totales</div>
              </button>
            );
          })}
        </div>
          ...
          return (
            <div className="space-y-4">
              <div className="bg-white rounded-xl border border-ink-100 shadow-card p-5">
                ...
                <p className="text-sm text-ink-500">{c.company}</p>
                <div className="grid sm:grid-cols-2 gap-x-8 gap-y-2 mt-4 text-sm">
                  <div><div className="text-[11px] uppercase text-ink-400 font-medium">Dirección</div><div className="text-ink-800">{c.address}</div></div>
                  {c.contacts.map((ct, i) => (
                    <div key={i}><div className="text-[11px] uppercase text-ink-400 font-medium">Contacto</div><div className="text-ink-800">{ct.name} · {ct.phone} · {ct.email}</div></div>
                  ))}
                </div>
                {c.notes && <p className="text-sm text-ink-500 mt-3 italic">{c.notes}</p>}
              </div>
